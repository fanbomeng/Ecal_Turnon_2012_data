...
//{
void makePairs_test_new2(int startN=%startN%,int endN=%endN%,int nEntries=-1, int iHalf=0, int nHalf=1,
	       TString dirOut="~/FANBOWORKINGAREA/CMSSW_6_2_0/src/L1Studies/EGamma/Macros/Efficiency/",
	       TString nameChain="produceNtuple/eIDSimpleTree", 
	       int iJson=-1, TString RunPhase="2011A", bool debug=false)
...
//  std::string endn = std::to_string(endN);
  ...
  vector<int> outfileN;
  TString Path="root://eoscms.cern.ch///store/user/fmeng/Crab3/DoubleElectron/crab_ECAL_LASER_CORRECTION_D/150608_175029/0000/";
  for(int i=startN;i<=endN;i++)
  {
      std::string startn = to_string(i);
      treeList.push_back(Path+"tree_testFastSim_TPG_sfgvb24_"+startn+".root");
