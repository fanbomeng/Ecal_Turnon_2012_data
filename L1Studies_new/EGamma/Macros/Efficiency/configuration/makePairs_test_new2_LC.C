...
//#include "tree_list_Run2012D_22Jan2013_v1.h"
// DATA TREE LISTS
...
//void makePairs_test_new(int startN, int endN,int nEntries=-1, int iHalf=0, int nHalf=1,
//	       TString dirOut="~/FANBOWORKINGAREA/CMSSW_6_2_0/src/L1Studies/EGamma/Macros/Efficiency/",
//	       TString dirIn="~/FANBOWORKINGAREA/CMSSW_6_2_0/src/L1Studies/EGamma/Macros/Efficiency/MakePairsTest/",
...
//{
void makePairs_test_new2_LC(int startN=%startN%,int endN=%endN%,int nEntries=-1, int iHalf=0, int nHalf=1,
	       TString dirOut="~/FANBOWORKINGAREA/CMSSW_6_2_0/src/L1Studies/EGamma/Macros/Efficiency/",
	       TString nameChain="produceNtuple/eIDSimpleTree", 
	       int iJson=-1, TString RunPhase="2011A", bool debug=false)
...
//  std::string startn = std::to_string(startn);
//  std::string endn = std::to_string(endN);
  vector<TString> treeList;
  vector<int> outfileN;
  TString Path="root://eoscms.cern.ch///store/user/fmeng/Crab3/DoubleElectron/crab_ECAL_LASER_CORRECTION_D_LC/150908_231822/0000/";
  for(int i=startN;i<=endN;i++)
  {
      std::string startn = to_string(i);
      treeList.push_back(Path+"tree_testFastSim_TPG_checknow_"+startn+".root");
      outfileN.push_back(i);
  }
  //treeList.push_back("~/FANBOWORKINGAREA/CMSSW_6_2_0/src/L1Studies/EGamma/Macros/Efficiency/tree_testFastSim_TPG_sfgvb24.root");
 // treeList=tree_list_Run2012D_ZElectron_22Jan2013_v1();
...
//  }
  ...
  for(int i = n1 ; i < n2 ; i++) {
    ...
    std::cout<<"give me the crect path   "<<treeList[i]<<std::endl;
    process(nameChain,treeList[i],outfileN[i],nEntries,dirOut,outlog,iJson,RunPhase,debug);
